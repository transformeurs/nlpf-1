import { ArrowPathIcon, BuildingOfficeIcon, CalendarIcon, CheckBadgeIcon, CheckCircleIcon, EnvelopeIcon, QuestionMarkCircleIcon, XCircleIcon } from "@heroicons/react/20/solid";
                ...
                {candidacies.map((offer, offerIdx) => (
                    <div key={offerIdx} className="flex rounded bg-white p-6 shadow-lg">
                        ...
                        <div className="flex flex-col">
                            <div>
                                <Button
                                    type={offer.status == 'En attente' ? ButtonType.PRIMARY : offer.status == 'Acceptée' ? ButtonType.SUCCESS : ButtonType.DANGER}
                                    size={ButtonSize.XL}
                                    label={offer.status == 'En attente' ? 'En attente' : offer.status == 'Acceptée' ? 'Acceptée' : 'Refusée'}
                                    rightIcon={offer.status == 'En attente' ? QuestionMarkCircleIcon : offer.status == 'Acceptée' ? CheckBadgeIcon : XCircleIcon}
                                    className="w-full"
                                />
                            </div>
                            <div>
                                <div className="mt-2 whitespace-nowrap text-sm text-gray-600">
                                    Candidature envoyée le {offer.createdAt.toLocaleDateString()}
                                </div>
                            </div>
                        </div>
                    </div>
